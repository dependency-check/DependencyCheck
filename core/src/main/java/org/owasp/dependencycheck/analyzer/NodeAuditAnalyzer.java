...
public class NodeAuditAnalyzer extends AbstractNpmAnalyzer {
    ...
    protected void analyzeDependency(Dependency dependency, Engine engine) throws AnalysisException {
        ...
        try {
            final List<Advisory> advisories = getAdvisories(packageLock, folder);
            final MultiValuedMap<String, String> dependencyMap = new HashSetValuedHashMap<>();
            collectDependencyVersions(folder, dependency, dependencyMap);
     ...
     */
    private List<Advisory> getAdvisories(File packageLock, File folder) throws AnalysisException {
        String key = null;
        if (cache != null) {
            try {
                key = Checksum.getSHA256Checksum(packageLock);
        ...
        final JSONObject auditReport = fetchNpmAuditReport(folder);
     ...
     * parsing the `npm audit` output
     */
    private JSONObject fetchNpmAuditReport(File folder) throws AnalysisException {
        final String report = executeNpmJsonPackageLockCommand(folder, "audit", "--audit-level=none");
     ...
     */
    private void collectDependencyVersions(File folder, Dependency dependency,
                                           MultiValuedMap<String, String> dependencyMap) throws AnalysisException {
        final String output = executeNpmJsonPackageLockCommand(folder, "ls", "--all");
        try {
     ...
     * its output read
     */
    private String executeNpmJsonPackageLockCommand(File folder, String... npmCommand) throws AnalysisException {
        try {
            final List<String> args = new ArrayList<>();
            args.add(npmPath);
            args.addAll(Arrays.stream(npmCommand).collect(Collectors.toList()));
            if (getSettings().getBoolean(Settings.KEYS.ANALYZER_NODE_AUDIT_SKIPDEV, false)) {
                args.add("--omit=dev");
            }
            ...
            final File tmpFile = getSettings().getTempFile("npm_" + npmCommand[0], "json");
            ...
            throw new AnalysisException(String.format("npm %s failure; this error can be ignored if you are not analyzing "
                    + "projects with an npm lockfile.", npmCommand[0]), ioe);
        }
    }
